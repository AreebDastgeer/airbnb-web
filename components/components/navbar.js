    ...
    <div className='parent   px-10 py-5 bg-white'>
      <div className='flex flex-row justify-between'>
        ...
        <div className='border-2 p-2 border-transparent rounded-full hover:shadow-l hover:bg-gray-200'>
          ...
          <div className="flex items-center space-x-2 border-2 p-2 rounded-full hover:shadow-xl"> <FaBars /><FaUser /></div>
        </div>
      </div>
      {showCalendar && (
        <div className='flex flex-col col-span-3 mx-auto mt-10 text-black'>
          <div className='calendar-container current:rose-800'>
            <DateRangePicker
        ...
        <div class="flex item-center border-b mb-4 text-black"><h2 class="text-2xl flex-grow font-semibold">Number of Guests</h2>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-5 mt-1">
        <path d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z">
        </path>
        </svg>
        <input class="w-12 pl-2 outline-none mb-1 text-rose-600 text-lg placeholder-rose-600" placeholder="1" min="1" max="999" type="number">
        </input>
        </div>
      <div class="flex">
      <button class="flex-grow text-gray-500" onClick={handleClose}>Cancel</button>
      <button class="flex-grow text-rose-600">Search</button>
      </div>
      </div>
      )}
    </div>
